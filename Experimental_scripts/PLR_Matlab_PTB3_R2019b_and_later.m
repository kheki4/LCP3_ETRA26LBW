...
outputDir = fullfile(baseDir,'data');
if ~isfolder(outputDir)
    mkdir(outputDir);
end

useTitta = true;
tittaDummyMode = false;

pupilEXT = PupilEXT();
pupilEXT.Enabled = true; % Easy way to turn off if you are debugging

pupilEXT.Method = 0;
pupilEXT.UDP_IP = '169.254.14.202';
...
pupilEXT.RecordingsPath = 'D:/PupilEXT_Recordings'; % A slash at the end is not necessary
pupilEXT.ParticipantName = participantName;
% pupilEXT.DataRecordingDelimiter = ';';
pupilEXT.ImageRecordingFormat = 'jpeg';
...
if useTitta
    ...
    EThndl.init();
    % EThndl.buffer.start('gaze'); % temporary, to get the cameras exposing
end
...
%% Calibrate using Titta and start recording
if useTitta
    ...
    % tStart = GetSecs();
    EThndl.sendMessage(sprintf('RECORDING STARTED'), tStart);
end
...
for j = 1 : numTrials
    ...
    pupilEXT.sendMessage(['TS_' num2str(tPres)]);

    if useTitta
        EThndl.sendMessage(sprintf('STIM_PRES_%s', num2str(j)), tPres);
    end
...
if useTitta
    tEnd = GetSecs();
    EThndl.sendMessage(sprintf('RECORDING STOPPED'), tEnd);

    EThndl.buffer.stop('gaze');
    dat = EThndl.collectSessionData();
    EThndl.saveData(dat, fullfile(outputDir, 'spectrum'), true);
end
